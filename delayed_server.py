    ...
    try:
        ...
        with open(output_file_path, 'wb') as output_file:
            while received_data < file_size:
                ...
                # introducing a deliberate delay which happens in some cases that is initialized
                # by the probability sampling. 0.4 means 40% chance, etc.
                if random.random() < probability_sampling:
                    delay_time = random.uniform(0.1, 1.0)
                    print(f"Introducing a delay of {delay_time} seconds.")
